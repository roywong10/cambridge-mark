...
9. **绝不点标注右键菜单的 `Reset marks & annotations`**(会清空整题分数 + 所有标注)。删单个标注只用右键的 `Remove annotation`。误弹出 Reset 确认框时点 `No`。
10. **先读+决策,再批量执行**。用缩略图通读两篇 → 写出「评分 + 标注清单」→ 才动手录分/标注。不要边滚边点(慢且易错)。详见 skill `mark-essay`。
